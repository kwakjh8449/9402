...
const DEMO = [
  { id:"r1", date:"2026-03-26", store:"이마트", total:17650, rawText:"", image:"", items:[
  ...
  { id:"r2", date:"2026-03-19", store:"홈플러스", total:18520, rawText:"", image:"", items:[
  ...
  { id:"r3", date:"2026-03-09", store:"롯데마트", total:12100, rawText:"", image:"", items:[
    { id:"i8", name:"서울우유 1L", normalizedName:"우유", qty:1, price:2890, category:"유제품" },
    { id:"i9", name:"바나나", normalizedName:"바나나", qty:1, price:4210, category:"과일" },
    { id:"i10", name:"라면 5입", normalizedName:"라면 5입", qty:1, price:5000, category:"가공식품" },
  ]},
];

const STORE_HINTS = ["이마트","홈플러스","롯데마트","트레이더스","코스트코","농협하나로마트","GS더프레시"];
const APP_KEY = "jangbu_receipts_v4";
const BACKUP_KEY = "jangbu_receipts_backup_v4";
const BUDGET_KEY = "jangbu_budget_v4";
...
function parseReceiptText(rawText){
  const text = String(rawText).replace(/[|]/g, " ").replace(/[₩￦]/g, "").replace(/,/g, "").replace(/\t+/g, " ").replace(/ +/g, " ");
  ...
  for (const line of lines){
    ...
    const rawName = line.slice(0, last.index).replace(/\d+/g, " ").replace(/[xX*]/g, " ").replace(/\s+/g, " ").trim();
    if (!rawName || rawName.length < 2) continue;
    items.push({ id:uid("item"), name:rawName, normalizedName:normalizeProductName(rawName), qty, price:last.value, category:categoryOf(rawName) });
  }
  const unique = [];
  const seen = new Set();
  ...
  return {
    ...
    rawText,
    image: "",
    items: unique.length ? unique : [{ id:uid("item"), name:"직접 입력 필요", normalizedName:"직접 입력 필요", qty:1, price:detectTotal(text), category:"확인필요" }],
  };
}
...
function buildChartPath(rows, width=320, height=140, pad=16){
...
function getPriceInsight(rows){
  if (!rows || rows.length < 2) return { level:"normal", label:"기록 부족", detail:"비교할 기록이 더 필요해." };
  const ordered = [...rows].sort((a,b)=>a.date.localeCompare(b.date));
  const latest = ordered[ordered.length - 1];
  const previousRows = ordered.slice(0, -1);
  const average = previousRows.reduce((sum, r)=>sum + r.price, 0) / previousRows.length;
  const min = Math.min(...previousRows.map(r => r.price));
  const rate = average ? (latest.price - average) / average : 0;
  if (latest.price >= average * 1.12 || latest.price >= min * 1.18) return { level:"high", label:"비싸게 샀어", detail:`평균보다 ${Math.round(rate * 100)}% 높아.` };
  if (latest.price <= average * 0.9) return { level:"good", label:"잘 샀어", detail:`평균보다 ${Math.round((1 - latest.price / average) * 100)}% 낮아.` };
  return { level:"normal", label:"보통 가격", detail:"평균 범위 안이야." };
}
...
function InsightPill({ insight }){
  ...
  const tone = insight.level === "high" ? "danger" : insight.level === "good" ? "good" : "default";
  return <Badge tone={tone}>{insight.label}</Badge>;
}
...
export default function Page(){
  ...
  const flatItems = useMemo(() => receipts.flatMap(receipt => receipt.items.map(item => ({ ...item, receiptId:receipt.id, date:receipt.date, store:receipt.store, total:receipt.total }))), [receipts]);
  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return flatItems;
    return flatItems.filter(item => [item.name, item.normalizedName, item.category, item.store, item.date, String(item.price)].join(" ").toLowerCase().includes(q));
  }, [flatItems, query]);
  ...
  const groups = useMemo(() => {
    ...
    return Array.from(map.entries()).map(([name, rows]) => {
      ...
      return { name, rows:ordered, latest, min, max, count:rows.length, insight:getPriceInsight(ordered) };
  ...
  const highPriceItems = useMemo(() => groups.filter(g => g.insight.level === "high").slice(0,3), [groups]);

  async function handleReceiptImage(files){
    const file = files?.[0];
    if (!file) return;
    const preview = URL.createObjectURL(file);
    setOcrOpen(true); setOcrProgress(5); setOcrStatus("영수증 이미지 준비 중");
    try {
    ...
    setReceipts(prev => prev.map(receipt => {
      ...
      return {
        ...
        items: receipt.items.map(item => {
          ...
          if (field === "name"){ next.category = categoryOf(value); next.normalizedName = normalizeProductName(value); }
          return next;
        }),
      };
    }));
  ...
  function addManualItem(){
    setManualReceipt(prev => ({ ...prev, items:[...prev.items, { id:uid("item"), name:"", qty:1, price:0, category:"기타", normalizedName:"" }] }));
  }
  ...
  function saveManualReceipt(){
    const cleanItems = manualReceipt.items.filter(i => i.name.trim()).map(i => ({
      ...i, qty:Number(i.qty || 1), price:Number(i.price || 0), category:categoryOf(i.name), normalizedName:normalizeProductName(i.name)
    }));
    const total = Number(manualReceipt.total || cleanItems.reduce((s,i)=>s + Number(i.price || 0), 0));
    const receipt = { id:uid("receipt"), date:manualReceipt.date || new Date().toISOString().slice(0,10), store:manualReceipt.store || "직접 입력", total, rawText:"", image:"", items: cleanItems.length ? cleanItems : [{ id:uid("item"), name:"직접 입력", normalizedName:"직접 입력", qty:1, price:total, category:"기타" }] };
    setReceipts(prev => [receipt, ...prev]);
    setSelectedId(receipt.id);
    setShowManual(false);
    setTab("receipts");
    setManualReceipt({ date:new Date().toISOString().slice(0,10), store:"", total:0, items:[{ id:uid("item"), name:"", qty:1, price:0, category:"기타", normalizedName:"" }] });
  }
    ...
    <div className="app-shell">
      <div className="topbar">
        <div>
          ...
          <div className="app-sub">영수증 OCR + 수기 입력 + 가격 인사이트 + 복구</div>
        </div>
      ...
      <div className="hero">
        ...
        <h1>영수증 스캔 및 입력</h1>
        <p>영수증이 없어도 구매 날짜, 금액, 품목을 직접 적을 수 있고, 자동 통합된 상품명으로 가격비교와 오늘 가격 인사이트까지 볼 수 있어.</p>
      ...
      <div className="tab-row">
        {[["home","홈"],["search","검색"],["compare","비교"],["receipts","영수증"],["backup","복구"]].map(([key,label]) => (
      ...
      {tab === "home" && (
        <div className="section-home">
          <div className="panel">
            <div className="row between"><div className="panel-title">최근 영수증</div><Badge tone="default">{receipts.length}건</Badge></div>
            <div className="list">
              {[...receipts].sort((a,b)=>b.date.localeCompare(a.date)).slice(0,5).map(receipt => (
                <button key={receipt.id} className="list-item" onClick={() => { setSelectedId(receipt.id); setTab("receipts"); }}>
                  <div><div className="strong">{receipt.store}</div><div className="muted">{receipt.date} · {receipt.items.length}개 품목</div></div>
          ...
          <div className="panel">
            <div className="row between"><div className="panel-title">오늘의 가격 인사이트</div><Badge tone="good">업그레이드됨</Badge></div>
            <div className="insight-stack">
              {highPriceItems.length ? highPriceItems.map(item => (
                <div className="insight-card" key={item.name}>
                  <div className="row between"><div className="strong">{item.name}</div><InsightPill insight={item.insight} /></div>
                  <div className="muted">{item.insight.detail}</div>
                  <div className="small muted">최근가 {won(item.latest.price)} · 최저가 {won(item.min)}</div>
                </div>
              )) : <div className="empty-block">아직 비싸게 산 품목 경고는 없어. 잘 사고 있는 중 👌</div>}
            </div>
          </div>

          <div className="panel wide">
            <div className="row between"><div className="panel-title">자동 통합 예시</div><Badge tone="default">비교 기준</Badge></div>
            <div className="muted">서울우유 1L / 우유 / 매일우유 1L 같은 이름은 비교용으로 “우유”로 묶이게 해놨어.</div>
            <div className="chip-wrap">{groups.slice(0,8).map(g => <span key={g.name} className="chip">{g.name}</span>)}</div>
          </div>
        </div>
      )}
        ...
        <div className="panel">
          ...
          <div className="table-wrap">
            <table className="table">
              <thead><tr><th>날짜</th><th>품목</th><th>매장</th><th>가격</th></tr></thead>
              <tbody>
                {filtered.length ? filtered.map(item => {
                  const itemGroup = groups.find(g => g.name === (item.normalizedName || item.name));
                  return (
                    <tr key={`${item.receiptId}-${item.id}`}>
                      <td>{item.date}</td>
                      <td><div className="strong">{item.name}</div><div className="muted small">통합명: {item.normalizedName} · {item.category}</div></td>
                      <td>{item.store}</td>
                      <td className="right"><div className="strong">{won(item.price)}</div>{itemGroup ? <div className="tiny-pill-wrap"><InsightPill insight={itemGroup.insight} /></div> : null}</td>
                    </tr>
                  );
      ...
      {tab === "compare" && (
        <div className="cards-grid">
          {groups.map(group => {
            const chart = buildChartPath(group.rows);
            return (
              <div key={group.name} className="panel compare-card">
                <div className="row between align-start">
                  <div><div className="panel-title">{group.name}</div><div className="muted">{group.count}번 구매</div></div>
                  <div className="card-actions"><InsightPill insight={group.insight} /><button className="ghost-btn" onClick={() => setShowCompare(group)}>상세</button></div>
                </div>
                ...
                <div className="chart-panel">
                  <svg viewBox="0 0 320 140" className="chart chart-pretty">
                    <defs>
                      <linearGradient id={`grad-${group.name}`} x1="0" x2="0" y1="0" y2="1">
                        <stop offset="0%" stopColor="currentColor" stopOpacity="0.28" />
                        <stop offset="100%" stopColor="currentColor" stopOpacity="0.04" />
                      </linearGradient>
                    </defs>
                    <path d={chart.area} fill={`url(#grad-${group.name})`} />
                    <path d={chart.line} fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" />
                    {chart.points?.map((p, idx) => <circle key={idx} cx={p.x} cy={p.y} r="4" className="chart-dot" />)}
                  </svg>
                  <div className="chart-labels"><span>{group.rows[0]?.date}</span><span>{group.rows[group.rows.length - 1]?.date}</span></div>
                </div>
                <div className="muted">{group.insight.detail}</div>
              </div>
            );
          })}
        </div>
      ...
      {tab === "receipts" && (
        <div className="section-grid">
            ...
            <div className="list">
              {receipts.map(receipt => (
                <button key={receipt.id} className={selectedId===receipt.id ? "list-item selected" : "list-item"} onClick={() => setSelectedId(receipt.id)}>
                  <div><div className="strong">{receipt.store}</div><div className="muted">{receipt.date}</div></div>
                  <div className="strong">{won(receipt.total)}</div>
                </button>
              ))}
            </div>
          ...
          <div className="panel">
            <div className="row between"><div className="panel-title">영수증 상세 수정</div>{selected ? <button className="danger-btn" onClick={() => deleteReceipt(selected.id)}>삭제</button> : null}</div>
            {selected ? (
              <>
                  ...
                  {selected.items.map(item => {
                    ...
                    return (
                      <div key={item.id} className="item-card">
                        ...
                        {itemGroup ? <div className="tiny-pill-wrap"><InsightPill insight={itemGroup.insight} /></div> : null}
                      </div>
                    );
                  })}
                </div>
      ...
      <div className="mobile-nav">
        {[["home","홈"],["search","검색"],["compare","비교"],["receipts","영수증"],["backup","복구"]].map(([key,label]) => (
          <button key={key} className={tab===key ? "mobile-tab active" : "mobile-tab"} onClick={() => setTab(key)}>{label}</button>
        ))}
      </div>
        ...
        <div className="modal-backdrop" onClick={() => setShowManual(false)}>
          <div className="modal" onClick={(e)=>e.stopPropagation()}>
            <div className="row between">
              <div><div className="panel-title">직접 구매내역 입력</div><div className="muted">영수증이 없어도 날짜, 금액, 품목 직접 입력 가능</div></div>
            ...
            <div className="row gap"><button className="secondary-btn" onClick={addManualItem}>품목 추가</button><button className="primary-btn" onClick={saveManualReceipt}>저장</button></div>
          </div>
        </div>
      )}
      ...
      {showCompare && (
        <div className="modal-backdrop" onClick={() => setShowCompare(null)}>
          <div className="modal large" onClick={(e)=>e.stopPropagation()}>
            <div className="row between">
              <div><div className="panel-title">{showCompare.name} 가격 비교</div><div className="muted">자동 통합된 상품명 기준으로 묶여 있어.</div></div>
              <button className="ghost-btn" onClick={() => setShowCompare(null)}>닫기</button>
            </div>
            <div className="compare-header"><InsightPill insight={showCompare.insight} /><span className="muted">{showCompare.insight.detail}</span></div>
            {(() => {
              const chart = buildChartPath(showCompare.rows, 760, 220, 22);
              return (
                <svg viewBox="0 0 760 220" className="chart chart-pretty big">
                  <defs>
                    <linearGradient id="compare-grad" x1="0" x2="0" y1="0" y2="1">
                      <stop offset="0%" stopColor="currentColor" stopOpacity="0.28" />
                      <stop offset="100%" stopColor="currentColor" stopOpacity="0.04" />
                    </linearGradient>
                  </defs>
                  <path d={chart.area} fill="url(#compare-grad)" />
                  <path d={chart.line} fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" />
                  {chart.points?.map((p, idx) => <circle key={idx} cx={p.x} cy={p.y} r="4.8" className="chart-dot" />)}
                </svg>
              );
            })()}
            <div className="table-wrap">
              <table className="table">
                <thead><tr><th>날짜</th><th>매장</th><th>수량</th><th>가격</th></tr></thead>
                <tbody>
                  {[...showCompare.rows].reverse().map((row, idx) => (
                    <tr key={`${row.date}-${idx}`}>
                      <td>{row.date}</td><td>{row.store}</td><td>{row.qty}</td><td className="right">{won(row.price)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
